...
class OrderCancelledMessage(betterproto.Message):
    ...
    order_ids: List[str] = betterproto.string_field(1)
...
@dataclass
class AssetExpiryMessage(betterproto.Message):
    assets: List["AssetExpiryMessageAsset"] = betterproto.message_field(1)
    # Timestamp of expiry
    timestamp: str = betterproto.string_field(2)


@dataclass
class AssetExpiryMessageAsset(betterproto.Message):
    """Assets that expired"""

    asset: str = betterproto.string_field(1)
    underlying: str = betterproto.string_field(2)
    lot_size: int = betterproto.int32_field(3)


@dataclass
class PositionMessage(betterproto.Message):
    # map from asset code to quantity held for that code
    positions: Dict[str, int] = betterproto.map_field(
...
class FeedMessage(betterproto.Message):
    ...
    asset_expiry_msg: "AssetExpiryMessage" = betterproto.message_field(9, group="msg")
    position_msg: "PositionMessage" = betterproto.message_field(10, group="msg")
...
@dataclass
class SwapSpec(betterproto.Message):
    """A swap, as sent from the client to the server"""
...
@dataclass
class SwapRequest(betterproto.Message):
    """A request to complete a swap"""

    # The credentials of the relevant competitor
    creds: "Credentials" = betterproto.message_field(1)
    # The swap to perform
    swap: "SwapSpec" = betterproto.message_field(2)
...
@dataclass
class SwapResponse(betterproto.Message):
    """A response to a request to perform a swap"""

    # Whether the reqest was successful (if false, the swap failed)
    ok: bool = betterproto.bool_field(1)
    # Message about why the swap may have failed
    message: str = betterproto.string_field(2)
...
@dataclass
class PositionRequest(betterproto.Message):
...
class PositionResponse(betterproto.Message):
    """Response to PositionRequest"""

    # Whether the request was successful (if false, the request failed)
    ok: bool = betterproto.bool_field(1)
    # map from asset code to quantity held for that code
    positions: Dict[str, int] = betterproto.map_field(
...
class ExchangeServiceStub(betterproto.ServiceStub):
    ...
    async def get_positions(
        ...
        return await self._unary_unary(
            "/utc_bot.ExchangeService/GetPositions",
            request,
            PositionResponse,
        )
